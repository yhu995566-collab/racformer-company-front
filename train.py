...
def main():
    ...
    val_loader = build_dataloader(val_dataset, samples_per_gpu=1, workers_per_gpu=cfgs.data.workers_per_gpu, num_gpus=world_size, dist=world_size > 1, shuffle=False)
    ...
    if world_size > 1:
        model = MMDistributedDataParallel(model, device_ids=[local_rank], broadcast_buffers=False)
    else:
    ...
    runner.register_training_hooks(
        cfgs.lr_config, cfgs.optimizer_config, cfgs.checkpoint_config, cfgs.log_config, cfgs.get("momentum_config", None), custom_hooks_config=cfgs.get("custom_hooks", None)
    )
    ...
    if cfgs.resume_from is not None:
        ...
        if cfgs.revise_keys is not None:
            load_checkpoint(model, cfgs.load_from, map_location="cpu", revise_keys=cfgs.revise_keys)
        else:
            load_checkpoint(
                model,
                cfgs.load_from,
