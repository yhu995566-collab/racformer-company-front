...
class RaCFormerSampling(BaseModule):
    ...
    def inner_forward(self, query_ray, query_feat, mlvl_feats, img_metas, d_region=0.1):
        ...
        if getattr(self, '_deploy_trt_scale_linear_barrier', False):
            scale_weights = tensorrt_fusion_barrier(scale_weights)
        scale_weights = scale_weights.view(B, Q, self.num_groups, self.num_frames, self.depth_num*self.num_points, self.num_levels).contiguous()
...
class BEVSampling(BaseModule):
    ...
    def inner_forward(self, query_ray, query_feat, bev_feats, img_metas, d_region=0.1):
        ...
        if getattr(self, '_deploy_trt_scale_linear_barrier', False):
            scale_weights = tensorrt_fusion_barrier(scale_weights)
        scale_weights = scale_weights.view(B, Q, self.num_heads, 1, self.num_levels, self.depth_num*self.num_points).contiguous()
        scale_weights = torch.softmax(scale_weights, dim=-1)
