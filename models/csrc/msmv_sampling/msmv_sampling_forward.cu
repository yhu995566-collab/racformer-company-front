...
__global__ void ms_deformable_im2col_gpu_kernel_c45(
    const float* feat_c4,
    const float* feat_c5,
    const int h_c4, const int w_c4,
    const int h_c5, const int w_c5,
    ...
    CUDA_KERNEL_LOOP(index, batch_size * num_query * channels) {  // n: bs x query x channels
        ...
        for (int p_col = 0; p_col < num_point; ++p_col) {
            ...
            const float weight_c4 = data_attn_weight[data_weight_ptr];
            const float weight_c5 = data_attn_weight[data_weight_ptr + 1];

            //const float h_im = loc_h * spatial_h - 0.5;  // align_corners = False
            //const float w_im = loc_w * spatial_w - 0.5;

            // C4 Feature
            float h_im = loc_h * (h_c4 - 1);  // align_corners = True
            float w_im = loc_w * (w_c4 - 1);
...
void ms_deformable_im2col_cuda_c45(
    const float* feat_c4,
    const float* feat_c5,
    const int h_c4, const int w_c4,
    const int h_c5, const int w_c5,
    ...
    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        printf("error in ms_deformable_im2col_cuda_c45: %s\n", cudaGetErrorString(err));
    }
}
